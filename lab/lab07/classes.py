...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        implemented = True
        player.draw()  #player.hand.extend([player.deck.draw() for _ in range(2)])
        player.draw()
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand:
            i=0
            while player.hand[i].power(opponent_card)!=max(j.power(opponent_card) for j in player.hand) :
                   i+=1
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if self.attack >= 0 or self.defense >= 0:
            player.hand.append(self) #player.hand += [self] 也可以这么写
            re_add = True
        # You should add your implementation above this.
        if re_add:
