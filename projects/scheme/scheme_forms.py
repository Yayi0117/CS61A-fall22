...
def do_and_form(expressions, env):
    ...
    for i in range(len(expressions)):
        if i == (len(expressions)-1):
            current_value = scheme_eval(expressions.first, env,True)
        current_value = scheme_eval(expressions.first, env)
        if not is_scheme_true(current_value):
...
def do_or_form(expressions, env):
    ...
    for i in range(len(expressions)):
        if i == (len(expressions)-1):
            current_value = scheme_eval(expressions.first, env,True)
        current_value = scheme_eval(expressions.first, env)
        if is_scheme_true(current_value):
            return current_value
        expressions = expressions.rest
    return current_value
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        vals = Pair(scheme_eval(binding.rest.first, env),vals)#难点 ：scheme_eval(binding.rest.first, env)
