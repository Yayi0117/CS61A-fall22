...
def perfect_square(score):
    base=1
    while base*base<score:
        base+=1
    return base*base==score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    m=0
    n=0
    standard=strategy(0,0)
    while m<100:
        while n<100:
            if standard!=strategy(m,n):
                return False
            else:
                n=n+1
        n=0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    averaged_score_max=averaged_score(1,dice)
    max_roll=1
    while j<10:
        j+=1
        m=averaged_score(j,dice)
        if m>averaged_score_max:
            averaged_score_max=m
            max_roll=j
    return max_roll    
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    incremental=square_update(0, score, opponent_score)-score
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if tail_points(opponent_score)>=make_averaged(roll_dice, total_samples=1000)(6):
        return 0
    else:
        score1=square_update(1, score, opponent_score)
        score2=square_update(2, score, opponent_score)
        if score2>score1:
            return 1
        else:
            return 2
