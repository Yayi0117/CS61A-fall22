...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            original=place.ant
            if original.is_container==True and original.ant_contained==None and self.is_container==False:
                original.ant_contained=self
            elif original.is_container==False and self.is_container==True and self.ant_contained==None:
    ...
    def nearest_bee(self):
        ...
        while current_place.entrance is not None:
            if not current_place.is_hive:
                if len(current_place.bees)>0 and k<=self.upper_bound and k>=self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            bees_damage+=self.damage
        for i in self.place.bees[:]: #? 这样的话，被修改的是复制的列表，原来的self.place.bees的内容没有被修改
            i.reduce_health(bees_damage)
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if len(self.place.bees)>0 :
            if self.turns_to_chew==0 or self.turns_to_chew > self.chewing_turns:    
                eatenbee=random_bee(self.place.bees)
                eatenbee.reduce_health(eatenbee.health)
                self.turns_to_chew=1
            else:
                if 0 < self.turns_to_chew < self.chewing_turns:
                    self.turns_to_chew+=1
                else:
                    self.turns_to_chew=0
# END Problem 7
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem EC
        self.trun_tracker=0
        while self.trun_tracker < 5:
            if target.gamestate.time%2==0:
