...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        branch1=totals_tree(end(left(m)))
        branch2=totals_tree(end(right(m)))
        return tree(total_weight(m),[branch1,branch2])
...
def has_path(t, word):
    ...
    if label(t)!=word[0]:
        ...
        if True in [has_path(b, word[1:]) for b in branches(t)]:
            return True
        else:
            return False
