...
def deep_map_mut(func, lnk):
    ...
    #if lnk.rest is not Link.empty: 这一句要不要都行，因为lnk is Link.empty已经在recursive base里考虑过了
...
def two_list(vals, counts):
    ...
    if len(vals) > 1:
        ...
        lnk = extend_link(lnk, two_list(vals[1:], counts[1:]))
    return lnk
    """
    solution:
    result = Link(None) 
...
def is_bst(t):
    ...
    def bst_min(t) :
        if t.is_leaf() : # t.is_leaf() : when invoke a bound method of an instance,don't foget the followed ()
            return t.label
